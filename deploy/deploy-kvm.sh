#!/usr/bin/env bash
# TDM ECR Car Wash landing page -> Profitcast KVM (root@187.127.149.216)
#
...
# hosts ~55 live client sites, so nothing here edits nginx or reloads it.
set -euo pipefail

HOST="root@187.127.149.216"
DOCROOT="/var/www/wash.lp.thedetailingmafiaecr.com"
LIVE_URL="https://wash.lp.thedetailingmafiaecr.com"
PREVIEW_URL="https://tdmecr-wash-preview.187.127.149.216.nip.io"
MODE="${1:-deploy}"
...
# excludes: this repo also holds README.md, audit-report.md, DEPLOYMENT.md, deploy/
# and a 4.4 MB zip, none of which belong on a public server. tdm-ecr-logo.png is
# the unused brand master. hero-banner.jpg does ship: it is the og:image that
# WhatsApp and Facebook fetch for link previews.
ship_files() {
  { echo index.html
    find assets -type f ! -name '.*' ! -name Thumbs.db ! -path assets/img/tdm-ecr-logo.png
  } | LC_ALL=C sort
}
...
case "$MODE" in
  ...
  deploy)
    ...
    ship_files | tar -czf - -T - | ssh_kvm "set -e; D=$DOCROOT
      T=\$(mktemp -d /var/www/.tdmecr-wash-new.XXXX)
